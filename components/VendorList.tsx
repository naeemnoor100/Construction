...
export const VendorList: React.FC = () => {
  ...
  const handleRecordPayment = async (e: React.FormEvent) => {
    ...
    if (amountNum > totalVendorHeadroom + 0.01) {
      alert(`Payment Forbidden: Amount exceeds vendor's total outstanding balance.`);
      return;
    }

    if (contextualMaxLimit !== null && amountNum > contextualMaxLimit + 0.01) {
      alert(`Strict Ledger Conflict: This specific purchase only has ${formatCurrency(contextualMaxLimit)} remaining.`);
      return;
    }
          ...
          <table className="w-full text-left min-w-[1000px]">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filteredVendors.map((vendor) => {
                ...
                // Optimized Row Engagement Calc
                const vendorPaymentsCount = payments.filter(p => p.vendorId === vendor.id).length;
                const activeSitesCount = useMemo(() => {
                   const vendorPayments = payments.filter(p => p.vendorId === vendor.id);
                   const vendorSupplies: any[] = [];
                   materials.forEach(m => m.history?.forEach(h => {
                     if (h.type === 'Purchase' && h.vendorId === vendor.id) vendorSupplies.push(h);
                   }));
                   const associatedProjectIds = new Set([...vendorSupplies.map(s => s.projectId), ...vendorPayments.map(p => p.projectId)]);
                   return projects.filter(p => associatedProjectIds.has(p.id) && p.status === 'Active').length;
                }, [vendor.id, payments, materials, projects]);
                 ...
                 <div className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-3xl border border-blue-100 dark:border-blue-800 flex items-center gap-4 animate-in slide-in-from-top-2">
                    ...
                    <div className="flex-1">
                       ...
                       <h4 className="text-sm font-bold text-slate-900 dark:text-white">
                         {materials.find(m => m.history?.some(h => h.id === paymentFormData.materialBatchId))?.name || 'Material Batch'}
                       </h4>
                    </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-800 rounded-[3rem] w-full max-w-6xl h-[92vh] shadow-2xl overflow-hidden flex flex-col mobile-sheet animate-in slide-in-from-bottom-8 duration-300">
               ...
               <div className="flex flex-wrap items-center gap-3">
                 <div className="bg-blue-50 dark:bg-blue-900/20 px-5 py-3 rounded-2xl border border-blue-100 dark:border-blue-800">
                    ...
                    <div className="flex items-center gap-2">
                       ...
                       <p className="text-sm font-black text-blue-700 dark:text-blue-300">{activeVendorStats.activeProjectsCount} Sites</p>
                    </div>
                 </div>
                 <div className="bg-emerald-50 dark:bg-emerald-900/20 px-5 py-3 rounded-2xl border border-emerald-100">
                    <p className="text-[8px] font-black text-emerald-600 uppercase tracking-widest mb-0.5">Total Settled</p>
                    <div className="flex items-center gap-2">
                 ...
                 <div className="bg-rose-50 dark:bg-rose-900/20 px-5 py-3 rounded-2xl border border-rose-100">
                    <p className="text-[8px] font-black text-rose-600 uppercase tracking-widest mb-0.5">Outstanding</p>
                    <div className="flex items-center gap-2">
            ...
            <div className="flex border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/20 shrink-0 px-4">
               ...
               <button onClick={() => setActiveDetailTab('payments')} className={`px-8 py-5 text-[10px] font-black uppercase tracking-widest transition-all ${activeDetailTab === 'payments' ? 'bg-white dark:bg-slate-800 text-emerald-600 border-b-4 border-emerald-600' : 'text-slate-400'}`}>Payments only</button>
               <button onClick={() => setActiveDetailTab('supplies')} className={`px-8 py-5 text-[10px] font-black uppercase tracking-widest transition-all ${activeDetailTab === 'supplies' ? 'bg-white dark:bg-slate-800 text-amber-600 border-b-4 border-amber-600' : 'text-slate-400'}`}>Bills only</button>
            </div>
                      ...
                      <table className="w-full text-left min-w-[850px]">
                        <thead className="bg-slate-50 dark:bg-slate-900 text-[10px] font-black text-slate-400 uppercase border-b border-slate-100">
                          <tr>
                            ...
                            <th className="px-8 py-5">Site / Project</th>
                            ...
                            <tr key={`${item.type}-${item.id}-${idx}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                                  ...
                                  <div className="flex items-center gap-3">
                                     ...
                                     <div>
                                        ...
                                        <span className={`text-[8px] font-black uppercase ${item.type === 'PAYMENT' ? 'text-emerald-500' : 'text-rose-500'}`}>
                                           {item.type === 'PAYMENT' ? 'Settlement In-Bound' : 'Material Arrival'}
                                        </span>
                                     </div>
                                  </div>
                               </td>
                               <td className="px-8 py-5 text-[10px] font-bold text-slate-600 uppercase tracking-tight">{projects.find(p => p.id === item.projectId)?.name || 'Direct Allocation'}</td>
                               ...
                               <td className="px-8 py-5 text-right">
                                  {item.type === 'PURCHASE' && !item.isFullyPaid ? (
                                    <button onClick={() => handleOpenPaymentModal(activeVendor, item.projectId, item.remainingBalance, item.id)} className="px-4 py-2 bg-emerald-600 text-white text-[9px] font-black uppercase tracking-widest rounded-xl shadow-md flex items-center gap-1.5 ml-auto"><DollarSign size={12} /> Pay Now</button>
                                  ) : item.type === 'PURCHASE' && item.isFullyPaid ? (
                                    <div className="flex items-center justify-end gap-1.5 text-emerald-500 text-[9px] font-black uppercase"><CheckCircle2 size={12} /> Paid</div>
                                  ) : (
                      ...
                      <table className="w-full text-left min-w-[600px]">
                        <thead className="bg-slate-50 dark:bg-slate-900 text-[10px] font-black text-slate-400 uppercase border-b border-slate-100">
                          <tr>
                            ...
                            <th className="px-8 py-5">Linked Material</th>
                              ...
                              <tr key={pay.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group/row">
                                ...
                                <td className="px-8 py-5">
                                   {matName ? (
                                     ...
                                     <span className="text-[9px] text-slate-300 font-bold uppercase tracking-widest italic">General Payment</span>
                                   )}
                                </td>
                                <td className="px-8 py-5 text-[11px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-tighter">{projects.find(p => p.id === pay.projectId)?.name || 'Direct Procurement'}</td>
                                <td className="px-8 py-5">
                                  <div className="flex flex-col">
                                ...
                                <td className="px-8 py-5 text-right">
                                  <div className="flex justify-end gap-2">
                                    ...
                                    <button onClick={() => deletePayment(pay.id)} className="p-2 text-slate-400 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                                  </div>
                                </td>
                              </tr>
                            );
                          ...
                          {vendorSupplies.map((supply, idx) => {
                            ...
                            return (
                              <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                                ...
                                <td className="px-8 py-5 text-[12px] font-black text-red-600 text-right">{formatCurrency(supply.estimatedValue)}</td>
                                <td className="px-8 py-5 text-right font-black text-emerald-600 text-[11px]">{formatCurrency(totalPaidForBatch)}</td>
                                <td className="px-8 py-5 text-right">
                                  <button disabled={isFullyPaid} onClick={() => handleOpenPaymentModal(activeVendor, supply.projectId, supply.estimatedValue - totalPaidForBatch, supply.id)} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase transition-all ml-auto ${isFullyPaid ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-emerald-600 text-white shadow-md'}`}>
                                    {isFullyPaid ? 'Settled' : 'Pay Balance'}
                                  </button>
                                </td>
                              </tr>
                            );
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-xl shadow-2xl overflow-hidden mobile-sheet animate-in slide-in-from-bottom-8 duration-300">
            ...
            <form onSubmit={(e) => {
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="e.g. Acme Construction Supplies" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-bold dark:text-white outline-none" value={formData.name} onChange={(e) => setFormData(p => ({ ...p, name: e.target.value }))} required />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <textarea placeholder="Shop #, Street, City, ZIP..." className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-bold dark:text-white outline-none" value={formData.address} onChange={(e) => setFormData(p => ({ ...p, address: e.target.value }))} rows={2} required />
              </div>
