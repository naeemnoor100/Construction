...
export const Inventory: React.FC = () => {
  ...
  const [procureData, setProcureData] = useState({
    materialId: '', newName: '', vendorId: vendors[0]?.id || '', projectId: projects.find(p => p.isGodown)?.id || projects[0]?.id || '', quantity: '', unit: stockingUnits[0] || 'Bag', costPerUnit: '', date: new Date().toISOString().split('T')[0], note: '', billNo: ''
  });
  ...
  const handleProcureStock = async (e: React.FormEvent) => {
    ...
    if (procureData.materialId === 'new') {
      ...
      await addExpense({ id: 'e-pro-' + Date.now(), date: procureData.date, projectId: procureData.projectId, vendorId: procureData.vendorId, amount: totalAmount, paymentMethod: 'Bank', category: 'Material', notes: procureData.note || `Procured ${qty} ${procureData.unit}. Bill: ${procureData.billNo}`, materialId: newId, materialQuantity: qty, inventoryAction: 'Purchase' });
    } else {
      await addExpense({ id: 'e-pro-' + Date.now(), date: procureData.date, projectId: procureData.projectId, vendorId: procureData.vendorId, amount: totalAmount, paymentMethod: 'Bank', category: 'Material', notes: procureData.note || `Procurement: ${qty} units. Bill: ${procureData.billNo}`, materialId: procureData.materialId, materialQuantity: qty, inventoryAction: 'Purchase' });
    }
  ...
  const handleRecordUsage = async (e: React.FormEvent) => {
    ...
    await addExpense({ 
      ...
      notes: `${usageData.notes || 'Site Consumption'}. Authorized by: ${usageData.responsible}`
    });
    setShowUsageModal(false);
    setUsageData(p => ({ ...p, quantity: '', notes: '', batchId: '', responsible: '' }));
  };
  ...
  const handleBulkProcureSubmit = async (e: React.FormEvent) => {
    ...
    for (const row of validRows) {
      ...
      await addExpense({
        ...
        notes: `Bulk Inward [Ref: ${bulkReference}]`
      });
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Manage and distribute project assets.</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search by asset name..." className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl text-sm font-bold outline-none dark:text-white" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
          ...
          <div key={mat.id} className="bg-white dark:bg-slate-800 p-5 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-4 active:scale-[0.98] transition-all group">
            <div className="flex justify-between items-start">
              <div className="flex gap-4 items-center">
                <div className="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-2xl flex items-center justify-center text-slate-500 font-black"><Package size={24} /></div>
                <div>
           ...
           <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-300">
              <div className="p-8 border-b border-slate-100 dark:border-slate-700 bg-blue-50/30 dark:bg-blue-900/20 flex justify-between items-center shrink-0">
                 <div className="flex gap-4 items-center">
                    ...
                    <div><h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter leading-none">Record Consumption</h2><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Deduct stock from site inventory</p></div>
                 </div>
              ...
              <form onSubmit={handleRecordUsage} className="p-8 space-y-5 overflow-y-auto no-scrollbar max-h-[75vh] pb-safe">
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Deduction Site</label>
                    ...
                    <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Value Date</label>
                    <input type="date" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={usageData.date} onChange={e => setUsageData(p => ({ ...p, date: e.target.value }))} required /></div>
                 </div>
                 
                 <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Select Material Batch (FIFO)</label>
                 <select className="w-full px-5 py-4 bg-white dark:bg-slate-900 border-2 border-blue-100 rounded-2xl font-bold dark:text-white outline-none appearance-none text-xs" value={`${usageData.materialId}|${usageData.batchId}`} onChange={e => { const [mId, bId] = e.target.value.split('|'); setUsageData(p => ({ ...p, materialId: mId, batchId: bId })); }} required>
                    <option value="|">Choose stock pool...</option>
                    {relevantMaterialsForSite.map((batch, idx) => (<option key={idx} value={`${batch.id}|${batch.batchId}`} className={batch.isLocal ? 'text-emerald-600 font-bold' : 'text-blue-500'}>{batch.name} / {batch.vendorName} / {batch.available.toLocaleString()} {batch.unit} Left</option>))}
                 </select></div>

                 <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Quantity</label>
                   <input type="number" step={allowDecimalStock ? "0.01" : "1"} required className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl font-black text-lg dark:text-white outline-none" value={usageData.quantity} onChange={e => setUsageData(p => ({ ...p, quantity: e.target.value }))} placeholder="0.00" /></div>
                   <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Staff / Responsible</label>
                   <div className="relative"><User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input type="text" className="w-full pl-10 pr-4 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={usageData.responsible} onChange={e => setUsageData(p => ({ ...p, responsible: e.target.value }))} placeholder="Person name..." required /></div></div>
                 </div>

                 <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Work Description / Remarks</label><textarea rows={2} className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={usageData.notes} onChange={e => setUsageData(p => ({ ...p, notes: e.target.value }))} placeholder="e.g. For slab casting..." /></div>
                 <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black shadow-2xl active:scale-95 transition-all text-sm uppercase tracking-widest mt-2">Confirm Site Consumption</button>
              </form>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
           <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-300">
              <div className="p-8 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex justify-between items-center shrink-0">
                 <div className="flex gap-4 items-center">
                    <div className="p-4 bg-slate-900 text-white rounded-2xl shadow-lg"><ShoppingCart size={24} /></div>
                    <div><h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter leading-none">Register Inward</h2><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Single asset procurement</p></div>
                 </div>
              ...
              <form onSubmit={handleProcureStock} className="p-8 space-y-5 overflow-y-auto no-scrollbar max-h-[75vh] pb-safe">
                ...
                {procureData.materialId === 'new' && <div className="grid grid-cols-2 gap-4 animate-in fade-in zoom-in-95"><input type="text" placeholder="Name" required className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl font-bold" value={procureData.newName} onChange={e => setProcureData(p => ({ ...p, newName: e.target.value }))} /><select className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl font-bold" value={procureData.unit} onChange={e => setProcureData(p => ({ ...p, unit: e.target.value }))}>{stockingUnits.map(u => <option key={u} value={u}>{u}</option>)}</select></div>}
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Storage Hub</label><select className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" value={procureData.projectId} onChange={e => setProcureData(p => ({ ...p, projectId: e.target.value }))} required>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Billing Supplier</label><select className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" value={procureData.vendorId} onChange={e => setProcureData(p => ({ ...p, vendorId: e.target.value }))} required><option value="">Select Vendor...</option>{vendors.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Quantity</label><input type="number" step={allowDecimalStock ? "0.01" : "1"} required className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-black text-lg" value={procureData.quantity} onChange={e => setProcureData(p => ({ ...p, quantity: e.target.value }))} /></div>
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Unit Rate (Rs.)</label><input type="number" step="0.01" required className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-black text-lg" value={procureData.costPerUnit} onChange={e => setProcureData(p => ({ ...p, costPerUnit: e.target.value }))} /></div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Value Date</label><input type="date" required className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" value={procureData.date} onChange={e => setProcureData(p => ({ ...p, date: e.target.value }))} /></div>
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-slate-400 uppercase px-1">Reference / Bill No.</label><div className="relative"><Hash size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-10 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" value={procureData.billNo} onChange={e => setProcureData(p => ({ ...p, billNo: e.target.value }))} placeholder="Invoice ID..." /></div></div>
                </div>

                <button type="submit" className="w-full bg-slate-900 text-white py-5 rounded-[2rem] font-black shadow-2xl transition-all active:scale-95 text-sm uppercase tracking-widest">Authorize Reception</button>
              </form>
           </div>
        </div>
      )}
      ...
      {showBulkModal && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] w-full max-w-6xl h-[90vh] shadow-2xl overflow-hidden flex flex-col mobile-sheet animate-in slide-in-from-bottom-8 duration-300">
            <div className="p-8 border-b border-slate-100 dark:border-slate-700 bg-emerald-50/20 flex justify-between items-center shrink-0">
              <div className="flex gap-4 items-center">
                 <div className="p-4 bg-emerald-600 text-white rounded-2xl shadow-lg"><Layers size={24} /></div>
                 <div><h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter leading-none">Bulk Stock Inward</h2><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Multi-asset reception ledger</p></div>
              </div>
              <button onClick={() => setShowBulkModal(false)} className="p-2 text-slate-400 hover:text-slate-900"><X size={32} /></button>
            </div>
            
            <div className="p-8 bg-slate-50/50 dark:bg-slate-900/30 border-b border-slate-100 dark:border-slate-700 flex flex-wrap gap-6 shrink-0">
              <div className="flex-1 min-w-[200px] space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase px-1">Global Supplier</label><select className="w-full px-5 py-4 bg-white dark:bg-slate-800 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={bulkGlobalVendor} onChange={e => setBulkGlobalVendor(e.target.value)}><option value="">Select Vendor...</option>{vendors.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></div>
              <div className="flex-1 min-w-[200px] space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase px-1">Destination Hub</label><select className="w-full px-5 py-4 bg-white dark:bg-slate-800 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={bulkGlobalProject} onChange={e => setBulkGlobalProject(e.target.value)}>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
              <div className="w-full sm:w-48 space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase px-1">Date</label><input type="date" className="w-full px-5 py-4 bg-white dark:bg-slate-800 border border-slate-200 rounded-2xl font-bold dark:text-white outline-none" value={bulkDate} onChange={e => setBulkDate(e.target.value)} /></div>
              <div className="w-full sm:w-64 space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase px-1">DC / GRN Ref</label><div className="relative"><FileText size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-10 pr-4 py-4 bg-white border border-slate-200 rounded-2xl font-bold" value={bulkReference} onChange={e => setBulkReference(e.target.value)} placeholder="Ref Number..." /></div></div>
            </div>

            <div className="flex-1 overflow-y-auto p-8 space-y-4 no-scrollbar">
              <div className="grid grid-cols-1 gap-4">
                 {bulkRows.map((row, idx) => (
                    <div key={row.id} className="bg-white dark:bg-slate-800 p-6 border border-slate-100 dark:border-slate-700 rounded-[2.5rem] shadow-sm flex flex-col md:flex-row gap-6 items-center animate-in zoom-in-95">
                       <div className="flex-1 w-full space-y-1.5">
                          <label className="text-[9px] font-black text-slate-400 uppercase px-1">Asset Class</label>
                          <select className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 rounded-xl font-bold text-sm outline-none" value={row.materialId} onChange={e => { const r = [...bulkRows]; r[idx].materialId = e.target.value; setBulkRows(r); }}>
                             <option value="">Choose material...</option>
                       ...
                       <div className="w-full md:w-32 space-y-1.5">
                          ...
                          <input type="number" step={allowDecimalStock ? "0.01" : "1"} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl font-black text-sm" value={row.quantity} onChange={e => { const r = [...bulkRows]; r[idx].quantity = e.target.value; setBulkRows(r); }} />
                       </div>
                       <div className="w-full md:w-40 space-y-1.5">
                          <label className="text-[9px] font-black text-slate-400 uppercase px-1">Unit Rate</label>
                          <input type="number" step="0.01" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl font-black text-sm" value={row.unitPrice} onChange={e => { const r = [...bulkRows]; r[idx].unitPrice = e.target.value; setBulkRows(r); }} />
                       </div>
                       <div className="w-full md:w-48 bg-slate-50 p-3 rounded-2xl border border-slate-100 text-center">
                          <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Line Total</p>
                          <p className="text-sm font-black text-slate-900">{formatCurrency((parseFloat(row.quantity) || 0) * (parseFloat(row.unitPrice) || 0))}</p>
                       </div>
                       <button onClick={() => setBulkRows(bulkRows.filter(i => i.id !== row.id))} className="p-4 bg-rose-50 text-rose-600 rounded-2xl hover:bg-rose-600 hover:text-white transition-all"><Trash2 size={20}/></button>
                    </div>
                 ))}
              </div>
              <button onClick={() => setBulkRows([...bulkRows, { id: Date.now().toString(), materialId: '', quantity: '', unitPrice: '', vendorId: '', projectId: '' }])} className="w-full py-6 border-2 border-dashed border-slate-200 rounded-[2.5rem] text-slate-400 hover:text-blue-600 hover:border-blue-300 font-black text-[11px] uppercase tracking-widest transition-all bg-slate-50/20">+ Add New Asset Line</button>
            </div>

            <div className="p-8 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row gap-4 shrink-0 pb-safe bg-white dark:bg-slate-800">
              <div className="flex-1">
                 <p className="text-[10px] font-black text-slate-400 uppercase px-1 mb-1">Batch Summary</p>
                 <p className="text-2xl font-black text-slate-900">Total: {formatCurrency(bulkRows.reduce((sum, r) => sum + (parseFloat(r.quantity) || 0) * (parseFloat(r.unitPrice) || 0), 0))}</p>
              </div>
              <button onClick={() => setShowBulkModal(false)} className="px-10 py-5 bg-slate-100 text-slate-600 rounded-[1.8rem] font-bold uppercase text-xs">Discard Batch</button>
              <button onClick={handleBulkProcureSubmit} className="px-12 py-5 bg-emerald-600 text-white rounded-[1.8rem] font-black uppercase text-xs shadow-2xl shadow-emerald-100 active:scale-95 transition-all">Authorize Inward Arrival</button>
            </div>
          </div>
        </div>
      )}
